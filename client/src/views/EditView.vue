  ...
  methods: {
    async getLetter() {
      try {
        ...
        this.cases.id = response.data.ltcase._id
        this.letter.gov_agency = response.data.letter.gov_agency
        this.letter.letter_no = response.data.letter.letter_no
        this.letter.source = response.data.letter.source
        this.letter.receive_at = response.data.letter.receive_at
        ...
        this.letter.sender.Id_card_created_at = response.data.letter.sender[0].Id_card_created_at
        this.letter.sender.Id_card_created_in = response.data.letter.sender[0].Id_card_created_in
        this.letter.sender.gender = response.data.letter.sender[0].gender
        this.letter.sender.ethnic = response.data.letter.sender[0].ethnic
        this.letter.sender.nationality = response.data.letter.sender[0].nationality
        this.letter.sender.address.province = response.data.letter.sender[0].address[0].province
        ...
        this.letter.sender.address.district = response.data.letter.sender[0].address[0].district
        ...
        this.letter.sender.address.commune = response.data.letter.sender[0].address[0].commune
        ...
        this.letter.times_case = response.data.letter.times_case
        this.cases.type = response.data.ltcase.case_type
        ...
        if (this.cases.type == 1) {
          ...
          this.pre_status_options.push({
            value: 7,
            label: 'Tố cáo tiếp - Đã có kết luận giải quyết',
          })
        ...
        this.cases.field = response.data.ltcase.field
        this.cases.content = response.data.ltcase.content
        this.letter.pre_status = response.data.letter.pre_status
      } catch (error) {
        console.log(error)
      }
    },
    async handleSave() {
      try {
        if (this.validateInput()) {
          ...
          formData.append('oldFile', JSON.stringify(this.oldFile))
          formData.append('deletedFile', JSON.stringify(this.deletedFile))

          const response = await axios.post('http://localhost:3000/letter/editLetter', formData, {
            headers: {
              'Content-Type': 'multipart/form-data',
            },
          })
          // const response = await axios.post('http://localhost:3000/letter/editLetter', {
          //   letter: this.letter,
          //   cases: this.cases,
          // })
          if (response.status == 200) {
            Swal.fire({
              title: 'Success',
              text: this.letter.status == 2 ? 'Đơn thư đã được gửi xử lý' : 'Đơn thư đã được sửa',
              icon: 'success',
            }).then((result) => {
    ...
    <div class="div-content">
      <div class="col-md-5">
        <p>
          ...
          <router-link class="link">Sửa đơn thư</router-link>
        </p>
      </div>
